...
- [x] **2.4 Verificar el despliegue automático.** Un push a `main` dispara el pipeline, publica exactamente el commit aprobado y `/api/salud` responde con ese commit desde la URL pública. *El workflow #12 terminó en verde en 3 min 35 s y la URL pública informó exactamente el commit `5dc35c6`.* Commit: `ci: completar despliegue automático`.
...
- [x] **6.1 Crear navegación interna.** `App` controla Portada, Menú, Configuración, Partida, Resultado y Temporada mediante estado de React, sin React Router. Commit: `feat: navegar entre pantallas del juego`.
- [x] **6.2 Implementar Portada, Menú y Configuración.** Portada con el botón de iniciar sobre la imagen de presentación; menú con las tarjetas de Eliminatoria, Liga e instrucciones; pantalla de configuración para elegir 1 o 2 jugadores, dificultad, equipos, estadio, perro activado o no y, en Eliminatoria, la meta de goles. Los equipos y estadios se piden al servidor, no se escriben en el cliente. Commit: `feat: crear pantallas de inicio y configuración`.
- [x] **6.3 Crear `usePartida`.** Centraliza estado remoto, carga, errores y llamadas `fetch`. Los componentes visuales no llaman a la API directamente. Commit: `feat: coordinar partida con usePartida`.
- [x] **6.4 Dibujar la cancha en SVG.** Cancha, arcos, 5 tapitas por equipo y pelota, ocupando la mayor parte de la pantalla. Nombres accesibles o `data-testid` estables. Orden de capas, de atrás hacia adelante: fondo del estadio, charcos, pelota y tapitas, caritas de emote sobre cada tapita, y los arcos **por encima de todo**, para que la pelota se vea entrando al arco. El arco es un solo archivo: para el lado contrario se voltea en espejo horizontal, no se rota. Commit: `feat: dibujar cancha interactiva`.
- [x] **6.5 Implementar apuntado.** Arrastrar desde una tapita propia para elegir dirección y fuerza, con una guía visual; cancelar correctamente un gesto inválido. Commit: `feat: apuntar y ejecutar tiros`.
- [x] **6.6 Reproducir recorridos.** `useAnimacion` con `requestAnimationFrame`. Bloquear nuevos tiros durante la reproducción; al final, aplicar el estado confirmado por Express. Commit: `feat: animar recorridos del servidor`.
- [x] **6.7 Mostrar información completa.** Marcador, turno con cuenta regresiva, minuto o goles rumbo a la meta, tiros de poder, errores y eventos (incluido el perro), siempre visibles sin abrir la consola. Commit: `feat: mostrar estado y retroalimentación`.
- [x] **6.8 Crear Resultado.** Ganador, empate o derrota, marcador final, revancha o volver al inicio. Commit: `feat: crear pantalla de resultado`.
...
- [x] **8.1 Estadios con efecto propio.** Añadir, sobre el estadio de referencia de Cochabamba (sin efecto especial), los demás estadios de `docs/reglas.md`, empezando por uno solo y agregando los demás si el tiempo lo permite. *Se hicieron los cuatro a la vez: agua y nieve comparten el mismo mecanismo y solo cambian sus números, en `dominio/estadios/configuracionEstadios.ts`.* Commit: `feat: estadios con efectos propios`.
- [x] **8.2 Rival simple.** Estrategia sencilla del servidor para el modo 1 jugador y el endpoint de turno rival. En lugar de tirar al azar, apunta como en el billar al punto de la pelota opuesto al arco que ataca, con la tapita propia mejor ubicada, fuerza según la distancia y error de puntería según la dificultad. *Se adelantó a la Fase 6: la pantalla de configuración ofrece 1 jugador, y sin rival el turno del servidor se vencía cada 15 segundos.* Commit: `feat: rival controlado por el servidor`.
- [x] **8.3 Rival por muestreo y dificultades.** Candidatos dirigidos hacia la pelota, función de puntuación, error de puntería y tres dificultades (fácil, medio, difícil) como configuración. *El tiro de billar del rival simple quedó como generador de los primeros candidatos, en `rival/billar.ts`.* Commit: `feat: rival por muestreo con tres dificultades`.
...
- [x] **9.7 Comprobar CI y producción, y medir tiempos.** Ejecutar las mismas capacidades en modo headless en GitHub Actions y contra la versión publicada. Volver a medir la duración total del pipeline y optimizar si pasa de 6 minutos. *La primera ejecución con las pruebas contra producción (commit `582d561`) terminó en verde en 215 segundos: las 25 pruebas corrieron antes del deploy y otra vez contra la URL pública.* Commit: `test: validar E2E en CI y producción`.
...
- [x] **10.1 Corregir la robustez de la interacción.** Revisar estados de carga, doble clic, botones durante solicitudes, gesto de apuntado cancelado, partida inexistente, contraste y foco visible. Añadir una comprobación E2E para cada regresión que se encuentre. Commit: `fix: mejorar robustez de la interacción`.
- [x] **10.2 Crear un modal accesible y confirmar la salida.** Reemplazar `window.confirm` por un componente reutilizable con fondo oscurecido, foco inicial, cierre con `Escape`, devolución del foco y acciones claras: continuar jugando o abandonar. La advertencia debe distinguir una partida suelta de una de temporada. Commit: `feat: confirmar salida con un modal accesible`.
- [x] **10.3 Implementar una pausa real.** Añadir Pausar/Reanudar y un modal con continuar o salir. Express debe conservar el tiempo restante del turno y de la Liga; React debe detener la animación, el turno automático del rival, el apuntado y las entradas hasta reanudar. Pausar no puede cambiar el marcador, consumir tiempo ni dejar solicitudes duplicadas. Documentar el contrato en `docs/api.md` y probar pausa, reanudación y salida. Commit: `feat: pausar y reanudar partidos`.
- [x] **10.4 Hacer rodar visualmente la pelota.** Mantener la imagen 2D y calcular su rotación acumulada en React a partir de la distancia recorrida entre cuadros (`ángulo = distancia / radio`). La rotación es solo presentación: no modifica posiciones ni resultados de Express, se detiene con la pelota y evita saltos al reiniciarse tras un gol o al moverla el perro. Respetar `prefers-reduced-motion`. Commit: `feat: animar el rodado de la pelota`.
- [x] **10.5 Mejorar la respuesta visual de la partida.** Dar más claridad a fuerza y dirección del tiro, estado del tiro de poder, cambio de turno, gol, pelota atrapada, aparición del perro y final del partido. Usar transiciones cortas y legibles, sin ocultar la cancha ni retrasar la interacción. Commit: `feat: mejorar animaciones y respuesta de la partida`.
- [x] **10.6 Unificar fondos, tarjetas y modales.** Convertir colores, sombras, radios y capas en variables reutilizables; reforzar la identidad de videojuego con gradientes, iluminación, viñeta y patrones CSS. Conservar las imágenes de Portada, Menú y estadios como protagonistas y evitar generar una imagen distinta para cada panel. Verificar al menos en 1280 × 720, 1366 × 768 y 1920 × 1080. Commit: `style: pulir fondos paneles y tarjetas`.
- [x] **10.7 Crear el sistema de audio.** Centralizar carga y reproducción, desbloquear audio después del primer gesto por las restricciones del navegador y añadir controles de silencio y volumen persistidos en `localStorage`. Separar música ambiental, interfaz, efectos y reacciones para que puedan mezclarse sin reproducir audios duplicados. La aplicación debe seguir siendo completamente jugable sin sonido. Incluye inventario y selección inicial de recursos con licencia, música al pulsar Iniciar y controles en menú/pausa; la conexión de jugadas queda en 10.8. Commit: `feat: agregar controles y sistema de audio`.
- [x] **10.8 Integrar música y efectos de la partida.** Usar música instrumental discreta, sin hinchada, cánticos, aplausos ni público porque el estadio está vacío. Conectar los recursos preparados de tiro de tapita, poder, gol, reacciones y transiciones; completar los pendientes de resortera y perro según `assets/audio/README.md`. Dispararlos desde acciones o eventos confirmados, no desde cada render de React; detener la música al pausar y evitar repeticiones al actualizar el estado. No inventar un evento de gol fallado ni de colisión si no existe una señal fiable. Conservar originales en `assets/audio/`, versiones web en `client/src/recursos/audio/` y registrar autoría, licencia y transformaciones. Commit: `feat: integrar sonidos de la partida`.
- [x] **10.9 Revisar accesibilidad, rendimiento y regresiones.** Respetar movimiento reducido y silencio, comprobar navegación por teclado, roles y nombres accesibles de los modales, revisar que audio y animaciones no degraden la fluidez y ejecutar lint, tipos, unitarias, E2E y build. Guardar evidencia visual y actualizar `docs/decisiones.md`. Commit: `test: validar el pulido audiovisual y de UX`.
